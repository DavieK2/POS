import AppErrors, { TError } from "#exceptions/app_error";
import * as E from 'fp-ts/lib/Either.js';
import * as TE from 'fp-ts/lib/TaskEither.js';
import BaseFeature from "../../../app/contracts/base_feature.js";
import { Auth } from "#services/pipeline_builder";
import { type } from "arktype";
import User from "#modules/user/models/user";
import ResponseMessage from "#services/response_message";

const rules = type({});

type ParamsType = typeof rules.infer

export default class SignOutFeature extends BaseFeature<TError, any> {

    async handle(params: ParamsType & Auth ): Promise<E.Either<TError, any>> {
         return await SignOutFeature.use<typeof params, typeof params>(params)
                                        .withAuth()
                                        .chain( (_,data) => this.signOut( data.__user as unknown as User ) )
                                        .chain( (_,__) => ResponseMessage.successMessage("Logged out"))
                                        .catchErrors()
                                        .handle<TError>({
                                            'Default': (err: TError) => TE.left(err),
                                        })
                                        .run();
    }

    signOut( user: User ){

        return TE.tryCatch(
            () =>  User.accessTokens.delete(user, user.currentAccessToken!.identifier),
            (err) =>  AppErrors.HandledError(err, "Could not sign out. Try again.", 400)
        )
    }

}