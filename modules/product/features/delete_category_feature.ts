...
export default class DeleteCategoryFeature extends BaseFeature<TError, any> {

    async handle(params: ParamsType & Auth ): Promise<E.Either<TError, any>> {
          return await DeleteCategoryFeature.use<typeof params, typeof params>(params)
                                            .withAuth()
                                            .chain( (_,data) => ValidationService.validate( { data, rules }) )
                                            .chain( (_,data) => validateCategory( data.categoryId ) )
                                            .chain( (_,data) => this.deleteCategory( data ) )
                                            .chain( (_,__) => ResponseMessage.successMessage("Category successfully deleted") )
                                            .catchErrors()
                                            .handle<TError>({
    ...
    deleteCategory =  ( opts: { categoryId: string }) => { 
        return  TE.tryCatch(
            () => dbq.deleteFrom("categories")
                     .where("id", "=", opts.categoryId )
                     .executeTakeFirst(),
            (err) => AppErrors.DBError(err, "There was an error deleting the category.")
        )
    }
}