<script lang="ts">
  ...
  let { categories, addItemToOrder, isHeld, activeOrder }: {
...
<div class="shrink-0 px-3 pt-3">
  <div class="flex items-center gap-2.5 bg-zinc-50 border border-zinc-200 rounded-xl px-3.5 py-3 focus-within:border-zinc-400 transition-colors {(isHeld || ! activeOrder ) ? 'opacity-50 grayscale-[0.5] pointer-events-none' : ''}">
    ...
    <input 
      ...
      placeholder={selectedCategory?.categoryName ? `Search in ${selectedCategory.categoryName}…` : "Search all products…"} 
      aria-label="Catalog search input" 
      class="bg-transparent text-sm text-zinc-700 placeholder-zinc-400 font-medium outline-none w-full disabled:cursor-not-allowed" 
    />
      ...
      {#each categories as cat}
        {#if (cat?.productCount || 0) > 0}
          ...
          <button 
            ...
            disabled={isHeld || !activeOrder}
            class="group relative flex flex-col p-3 rounded-[14px] border border-zinc-200 bg-white hover:border-zinc-300 hover:shadow-sm transition-all text-left overflow-hidden active:scale-[0.98] disabled:active:scale-100" 
            aria-label="Browse {cat.categoryName} category — {count} items">
            <div class="flex items-start justify-between mb-2.5">
              ...
              <span class="text-xs font-bold {meta.color} {meta.bg} border {meta.border} rounded-full px-2 py-0.5 shrink-0">{count}</span>
            </div>
  ...
  {#if searchResults.length > 0}
    <div class="flex items-center justify-between px-4 py-2 sticky top-0 bg-white/95 backdrop-blur-sm z-10 border-b border-zinc-100">
      {#if selectedCategory}
        ...
        <span class="text-sm font-bold {meta.color} flex items-center gap-1.5"><span>{meta.emoji}</span>{selectedCategory.categoryName}</span>
      {:else}
    ...
    <div class="flex flex-col px-3 py-3 gap-1">
      {#each searchResults as p (p.id)}
        ...
        <button
            ...
            disabled={isHeld || !activeOrder}
            class="flex items-center gap-3 px-3 py-2.5 rounded-[11px] border border-transparent hover:bg-zinc-50 hover:border-zinc-200 transition-all text-left cursor-pointer w-full group active:scale-[0.99] overflow-hidden relative disabled:cursor-not-allowed disabled:active:scale-100"
