...
>
  ...
  <main class="flex flex-1 overflow-hidden min-h-0" inert={anyModalOpen || undefined}>
            ...
            {#if searchQuery.trim() === ""}
              <div class="flex-1 flex flex-col items-center justify-center p-5 gap-6">
                <button
                  ...
                  class="relative flex-shrink-0 rounded-2xl max-w-min min-w-max flex items-center p-5 gap-5 overflow-hidden transition-all duration-500 active:scale-[0.98]
    ...
    {scanFlash ? 'scan-flash' : ''}"
                >
                  <div class="relative w-[60px] h-[60px] flex-shrink-0 flex items-center justify-center">
                    ...
                    <div class="flex items-end justify-center gap-[2px] h-[18px] relative z-10">
                ...
                <div class="flex flex-col items-center gap-1 z-10 relative">
                  <div class="flex items-center gap-[5px]">
                    <span class="w-[5px] h-[5px] rounded-full {scanning ? 'bg-green-400 animate-ping' : 'bg-green-400 animate-pulse'}"></span>
                    <span class="text-xs font-medium capitalize tracking-[0.1em] {scanning ? 'text-green-300' : 'text-zinc-500'}">{scanning ? "Reading…" : "Scanner ready"}</span>
                  </div>
                </div>

                <div class="text-center max-w-[260px] mt-2">
            ...
            {:else}
              <div class="flex items-center justify-between px-4 py-2 sticky top-0 bg-white/95 backdrop-blur-sm z-10 border-b border-zinc-100">
                ...
                <span class="text-[11px] font-medium text-zinc-400">{filteredCatalog.length} item{filteredCatalog.length !== 1 ? "s" : ""}</span>
              </div>
              ...
              {:else}
                <div class="flex flex-col px-3 py-3 gap-[4px]">
                  {#each filteredCatalog as p (p.id)}
                    {@const meta = categoryMeta[p.category] ?? { emoji: "📦", color: "text-zinc-600", bg: "", border: "", hex: "#71717a" }}
                    <button onclick={() => addItem(p.id)} class="flex items-center gap-3 px-3 py-2.5 rounded-[11px] border border-transparent hover:bg-zinc-50 hover:border-zinc-200 transition-all text-left cursor-pointer w-full group active:scale-[0.99] overflow-hidden relative" aria-label="Add {p.name} — {fmt(p.price)}">
                      <div class="absolute left-0 top-[20%] bottom-[20%] w-[3px] rounded-r-full opacity-60" style="background:{meta.hex}"></div>
                      <div class="w-9 h-9 rounded-[8px] overflow-hidden flex-shrink-0 bg-zinc-100 ml-2">
                        <img src={p.img} alt="" class="w-full h-full object-cover" />
                      </div>
                      <div class="flex-1 min-w-0">
                        <p class="text-[13.5px] font-semibold text-[#0A0A0A] truncate leading-snug">{p.name}</p>
                        <p class="text-[11px] text-zinc-400 mt-[2px]">{meta.emoji} {p.unit} &bull; <span class="font-['DM_Mono',monospace]">#{p.barcode}</span></p>
                      </div>
                      <div class="flex items-center gap-2 flex-shrink-0">
                        <span class="font-['DM_Mono',monospace] text-[13px] font-medium text-zinc-600">{fmt(p.price)}</span>
                        <div class="w-6 h-6 bg-zinc-200 group-hover:bg-[#0A0A0A] rounded-full flex items-center justify-center flex-shrink-0 transition-colors" aria-hidden="true">
          ...
          <div class="flex-shrink-0 p-3 border-t border-zinc-100 bg-white">
            <div class="flex items-center gap-2.5 bg-zinc-50 border border-zinc-200 rounded-[12px] px-3.5 py-3 focus-within:border-zinc-400 transition-colors">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path d="M8 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V8M8 21H7.8C6.11984 21 5.27976 21 4.63803 20.673C4.07354 20.3854 3.6146 19.9265 3.32698 19.362C3 18.7202 3 17.8802 3 16.2V16M21 8V7.8C21 6.11984 21 5.27976 20.673 4.63803C20.3854 4.07354 19.9265 3.6146 19.362 3.32698C18.7202 3 17.8802 3 16.2 3H16M21 16V16.2C21 17.8802 21 18.7202 20.673 19.362C20.3854 19.9265 19.9265 20.3854 19.362 20.673C18.7202 21 17.8802 21 16.2 21H16M3 12H3.01M7.5 12H7.51M16.5 12H16.51M12 12H12.01M21 12H21.01" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
              <input bind:value={searchQuery} onkeydown={handleSearchKeydown} type="text" placeholder="Type barcode manually" aria-label="Barcode search input" class="bg-transparent text-[14px] text-zinc-700 placeholder-zinc-400 font-normal outline-none w-full tracking-wider" />
              {#if searchQuery}
                <button onclick={() => (searchQuery = "")} aria-label="Clear search" class="text-zinc-400 hover:text-zinc-600 flex-shrink-0 transition-colors">
      ...
      {:else}
        <div class="flex-shrink-0 px-3 pt-3">
          <div class="flex items-center gap-2.5 bg-zinc-50 border border-zinc-200 rounded-[12px] px-3.5 py-3 focus-within:border-zinc-400 transition-colors">
            ...
            <input bind:value={searchQuery} type="text" placeholder={selectedCategory ? `Search in ${selectedCategory}…` : "Search all products…"} aria-label="Catalog search input" class="bg-transparent text-[14px] text-zinc-700 placeholder-zinc-400 font-medium outline-none w-full" />
            {#if searchQuery || selectedCategory}
              <button
                ...
                class="text-zinc-400 hover:text-zinc-600 flex-shrink-0 transition-colors"
              >
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            {/if}
        ...
        {#if selectedCategory}
          {@const meta = categoryMeta[selectedCategory] ?? { emoji: "📦", color: "text-zinc-700", bg: "", border: "", hex: "" }}
          <div class="flex-shrink-0 px-3 pt-2.5">
            <button
              onclick={() => {
                selectedCategory = null;
                searchQuery = "";
            ...
            >
              ...
              <span class={meta.color}>{meta.emoji} {selectedCategory}</span>
            </button>
          </div>
        {/if}

        <div class="flex-shrink-0 h-px bg-zinc-100 mt-2.5"></div>

        <div class="flex-1 overflow-y-auto custom-scrollbar min-h-0">
          {#if searchQuery.trim() === "" && selectedCategory === null}
            <div class="p-3 grid grid-cols-2 gap-2">
              {#each categories as cat}
                {@const meta = categoryMeta[cat] ?? { emoji: "📦", color: "text-zinc-700", bg: "bg-zinc-50", border: "border-zinc-200", hex: "#71717a" }}
                {@const count = catalog.filter((p) => p.category === cat).length}
                {@const previews = catalog.filter((p) => p.category === cat).slice(0, 3)}
                <button onclick={() => selectCategory(cat)} class="group relative flex flex-col p-3 rounded-[14px] border border-zinc-200 bg-white hover:border-zinc-300 hover:shadow-sm transition-all text-left overflow-hidden active:scale-[0.98]" aria-label="Browse {cat} category — {count} items">
                  <div class="flex items-start justify-between mb-2.5">
                    <div class="w-10 h-10 rounded-[10px] flex items-center justify-center text-[20px] {meta.bg} border {meta.border} flex-shrink-0">{meta.emoji}</div>
                    <span class="text-[11px] font-bold {meta.color} {meta.bg} border {meta.border} rounded-full px-2 py-[2px] flex-shrink-0">{count}</span>
                  </div>
                  <p class="text-[13px] font-bold text-zinc-800 mb-2">{cat}</p>
                  <div class="flex gap-1">
                    {#each previews as cp}
                      <div class="w-[26px] h-[26px] rounded-[5px] overflow-hidden bg-zinc-100 border border-zinc-200 flex-shrink-0">
                        <img src={cp.img} alt="" class="w-full h-full object-cover" />
                      </div>
                    {/each}
                    {#if count > 3}
                      <div class="w-[26px] h-[26px] rounded-[5px] bg-zinc-100 border border-zinc-200 flex items-center justify-center flex-shrink-0">
                        <span class="text-[9px] font-bold text-zinc-400">+{count - 3}</span>
                      </div>
                    {/if}
                  </div>
          ...
          {:else}
            <div class="flex items-center justify-between px-4 py-2 sticky top-0 bg-white/95 backdrop-blur-sm z-10 border-b border-zinc-100">
              {#if selectedCategory}
                {@const meta = categoryMeta[selectedCategory] ?? { emoji: "📦", color: "text-zinc-600", bg: "", border: "", hex: "" }}
                <span class="text-[12px] font-bold {meta.color} flex items-center gap-1.5"><span>{meta.emoji}</span>{selectedCategory}</span>
              {:else}
