<script lang="ts">
    ...
    import type { DropDownOptions, ProductFormData } from "../main/types";
        ...
        let { closeEditModal, formData = $bindable(), handleEditProduct, categoryOptions } : {
            closeEditModal: () => void,
...
<div
    ...
    onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') closeEditModal(); }}
  >
    <div
      ...
      <form onsubmit={handleEditProduct} class="p-6 space-y-4">
        ...
        <div>
          <label for="edit-category-dropdown" class="block text-sm font-medium text-neutral-700 mb-1">Category *</label>
            <Dropdown
                selected={categoryOptions.find( option => option.value === formData.category) || categoryOptions[0]}
