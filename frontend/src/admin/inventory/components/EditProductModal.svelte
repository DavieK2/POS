...
>
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-scale-in" role="dialog" aria-modal="true" aria-labelledby="edit-modal-title" tabindex="-1" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
    <div class="px-6 py-4 border-b border-neutral-200 flex items-center justify-between">
      <h2 id="edit-modal-title" class="text-lg font-semibold text-black">Edit Product</h2>
    ...
    <form onsubmit={handleEditProduct} class="p-6 space-y-4">
      ...
      <div>
        <label for="edit-category-dropdown" class="block text-sm font-medium text-neutral-700 mb-1">Category *</label>
        <Dropdown selected={categoryOptions.find((option) => option.value === formData.category) || categoryOptions[0]} options={categoryOptions} onSelect={(option) => (formData.category = option.value)} />
      </div>
